...
sourceCpp("/media/haiyanwu/HDD2/xinyi_RanDishonesty/behav_ddm/fitDDMs/mDDM_Rcpp.cpp")
...
# define fitting functions ---- logliklihood_mDDM

ll_mddm <- function(params, subjdata, md, cd) {
  ...
  for (i in 1:length(md)) {
    ...
    rts = mddm_parallel(drate_m,drate_c,nDT,md[i],cd[i],3000)
    #rts = rts[rts!=0]
...
fitSub <- function(subj_nr, alldata) {
  ...
  fit_subj = DEoptim(ll_mddm, lower, upper, DEoptim.control(itermax = 150), subjdata=subjdata, md=data1$error.correct,cd=data1$former_diff)
...
fitsF=fits
save(fitsF, file="/media/haiyanwu/HDD2/xinyi_RanDishonesty/behav_ddm/fitting_code/results_mddm/fits_all_mddm.RData")
write.csv(fitsF, file = "/media/haiyanwu/HDD2/xinyi_RanDishonesty/behav_ddm/fitting_code/results_mddm/fits_all_mddm.csv")

fitsF = as.data.frame(fitsF,ncol=9,byrow=TRUE)
names(fitsF)<-c("drate_m", "drate_c", 'nDT',"LL", "BIC", "AIC",'sub_nr')
